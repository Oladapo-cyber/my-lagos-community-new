...
const MENU_ITEMS: MenuItem[] = [
  { id: 'main-header', label: 'MAIN', isSection: true },
...
export const AdminLayoutWrapper = ({ children }: AdminLayoutWrapperProps) => {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-gray-50">
      {/* Top Navbar */}
      <nav className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 z-40 shadow-sm">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden border border-gray-200">
               <span className="font-bold text-gray-600">{getInitials()}</span>
            </div>
             <button
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <aside
          className={`w-64 bg-[#1652ff] text-white flex flex-col fixed lg:relative h-[calc(100vh-4rem)] lg:h-auto transition-transform duration-300 z-30 ${
              ...
              if (item.isSection) {
                return (
                  <div key={item.id} className="pt-4 pb-2">
                    <h3 className="text-xs font-bold text-blue-200 uppercase tracking-widest px-4">{item.label}</h3>
                  </div>
                );
              }
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left text-sm font-medium transition-colors ${
                    isActive
                      ? 'bg-[#0D3567] text-white shadow-md'
                      : 'text-blue-100 hover:bg-blue-500/30 hover:text-white'
                  }`}
                >
                  {Icon && <Icon size={20} className={isActive ? 'text-white' : 'text-blue-300'} />}
