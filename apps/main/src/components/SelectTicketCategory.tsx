...
type TicketType = 'regular' | 'vip' | 'gold' | 'platinum';
...
export const SelectTicketCategory: React.FC = () => {
  const { eventId } = useParams<{ eventId: string }>();
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState<TicketType | null>(null);
  const [attendees, setAttendees] = useState<string>('');
  const [error, setError] = useState<string>('');

  const event = EVENTS_DATA.find(e => e.id === Number(eventId));
  ...
  const ticketOptions: { type: TicketType; label: string }[] = [
  ...
  const handleProceed = () => {
    ...
    const qty = parseInt(attendees, 10);
    if (!attendees || isNaN(qty) || qty < 1 || qty > 10) {
      setError('You can only buy a minimum of 1 ticket, maximum of 10 tickets at a time');
      return;
    }

    const pricePerTicket = event.ticketPrices[selectedCategory];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#f5f5f5] flex flex-col overflow-y-auto">
      {/* Header */}
      <div className="flex items-center justify-between px-8 pt-6 pb-4">
        <div className="cursor-pointer" onClick={() => navigate(`/events/${eventId}`)}>
          <BrandLogo />
        </div>
        <button
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-10 text-center">
          Select a Ticket Category to proceed
        </h1>
          ...
          {ticketOptions.map(({ type, label }) => {
            ...
            return (
              <button
                ...
                className={`border rounded-lg py-5 px-6 text-center font-bold text-sm transition-all ${
                  isSelected
                    ? 'border-blue-600 bg-blue-50 ring-2 ring-blue-500'
                    : 'border-dashed border-gray-400 bg-white hover:border-gray-600'
                }`}
              >
                <span className="text-gray-900">{label}</span>
                <span className="text-gray-400 mx-1">|</span>
                <span className="text-[#FF4500]">
                  {formatPrice(event.ticketPrices[type])}
                </span>
              </button>
            );
          })}
        ...
        <div className="w-full max-w-sm mb-2">
          <label className="block text-center text-sm text-gray-700 mb-2">
            Number of Attendees
          </label>
          <input
            type="number"
            ...
            className="w-full border border-gray-300 rounded px-4 py-3 text-center text-gray-900 focus:outline-none focus:border-blue-500"
            placeholder=""
          />
        </div>

        {/* Warning / error */}
        <p className="text-[#FF4500] text-sm text-center mb-8 max-w-sm">
          You can only buy a minimum of 1 ticket, maximum of 10 tickets at a time
        </p>
        {error && error !== 'You can only buy a minimum of 1 ticket, maximum of 10 tickets at a time' && (
          <p className="text-red-600 text-sm text-center mb-4 -mt-6">{error}</p>
        )}

        {/* Proceed button */}
        <button
          onClick={handleProceed}
          className="border border-gray-800 text-gray-900 font-bold text-sm uppercase px-12 py-3 hover:bg-gray-900 hover:text-white transition-colors tracking-widest"
        >
          PROCEED &gt;&gt;
        </button>
      </div>
...
export default SelectTicketCategory;
