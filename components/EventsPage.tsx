            ...
            {CATEGORIES.map((cat, idx) => (
              ...
              >
                <cat.icon className={`w-4 h-4 transition-colors
                  ${activeCategory === cat.label ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}
                `} />
                <span className={`text-xs font-semibold transition-colors
                  ${activeCategory === cat.label ? 'text-blue-700' : 'text-gray-500 group-hover:text-gray-800'}
                `}>
                  {cat.label}
                </span>
              </div>
